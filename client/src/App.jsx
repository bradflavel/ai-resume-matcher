...
function App() {
  ...
  // falls back to localhost when VITE_API_URL is not set (local dev)
  const API_BASE = import.meta.env.VITE_API_URL || 'http://localhost:3001';

  // match the OS theme on first load
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    // don't clear the old result here, we want it visible if the new call fails
